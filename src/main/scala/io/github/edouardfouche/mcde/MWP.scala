  ...
  def twoSample(index: PreprocessedData, reference: Int, indexSelection: Array[Boolean]): Double = {
    ...
    def getStat(cutStart: Int, cutEnd: Int): Double = {
      ...
      if (n1 == 0 | n1 == cutLength) {
        ...
        val correction = (corrMax.toDouble - corrMin.toDouble) / (cutLength.toDouble * (cutLength.toDouble - 1))

        val std_input_1: Double = n1.toDouble * n2.toDouble / 12.0
        val std_input_2 = cutLength + 1 - correction
        val std = math.sqrt(std_input_1 * std_input_2) // handle ties https://en.wikipedia.org/wiki/Mann%E2%80%93Whitney_U_test
