...
def prepare_df(data_dict):
    ...
    for key in data_dict:
        ...
        result_mean = running_mean(_result[sorted_indices], window_size=ws)
        utility_mean = running_mean(_utility[sorted_indices], window_size=ws)
        quality_mean = running_mean(_quality[sorted_indices], window_size=ws)
        iterations_mean = running_mean(_iterations[sorted_indices], window_size=ws)
        variance_mean = running_mean(_variance[sorted_indices], window_size=ws)
        m_mean = running_mean(_m[sorted_indices], window_size=ws)
        M_mean = running_mean(_M[sorted_indices], window_size=ws)
        tcs_mean = running_mean(_tcs[sorted_indices], window_size=ws)
        t1_mean = running_mean(_t1[sorted_indices], window_size=ws)

        sorted_indices = np.argsort(_duration)
        duration_std = running_std(_duration[sorted_indices], window_size=ws)
        result_std = running_std(_result[sorted_indices], window_size=ws)
        utility_std = running_std(_utility[sorted_indices], window_size=ws)
        quality_std = running_std(_quality[sorted_indices], window_size=ws)
        iterations_std = running_std(_iterations[sorted_indices], window_size=ws)
        variance_std = running_std(_variance[sorted_indices], window_size=ws)
        m_std = running_std(_m[sorted_indices], window_size=ws)
        M_std = running_std(_M[sorted_indices], window_size=ws)
        tcs_std = running_std(_tcs[sorted_indices], window_size=ws)
        t1_std = running_std(_t1[sorted_indices], window_size=ws)

        tcs_t1_std = running_std(_tcs[sorted_indices] / _t1[sorted_indices], window_size=ws)
        t1_tcs_std = running_std(_t1[sorted_indices] / _tcs[sorted_indices], window_size=ws)
