...
def default_snapshot(default_score: float):
    return Snapshot(value=default_score, round=0, total_iterations=0, global_time=0.0, time_on_target=0.0,
                    ...
                    t_switch=np.nan, t1=np.nan, derivation_1st=np.nan, derivation_2nd=np.nan,
                    efficiency=np.nan, quality=0.0)
...
class Snapshot:
    def __init__(self, value: float,
                 ...
                 t_switch: float, t1: float, efficiency: float, quality: float):
        self.value = value
        self.round = round
...
def snapshots_to_df(snapshots: List[List[List[Snapshot]]]) -> pd.DataFrame:
    ...
    cols = ["rep", "round", "context_change", "target", "value", "m", "M",
            ...
            "derivation_1st", "derivation_2nd", "efficiency", "quality"]
        ...
        for context_change, snaps in enumerate(results):
            for target, snap in enumerate(snaps):
                new_res = [
                    ...
                    snap.derivation_1st, snap.derivation_2nd, snap.efficiency, snap.quality
                ]
                df.append(new_res)
